...
class NetscapeBookmarkUtils
{
    ...
    )
    {
		...
		error_log($filename);
		error_log($filesize);
		error_log($importCount);
		error_log($overwriteCount);
		error_log($skipCount);
    ...
    public static function import($post, $files, $linkDb, $pagecache)
    {
		...
		{
			error_log('Import HTML file does not contain the necessary NETSCAPE tag');
		...
		// https://raw.githubusercontent.com/kafene/netscape-bookmark-parser/master/NetscapeBookmarkParser.php
