...
class NetscapeBookmarkParser
{
    ...
    public function parseString($bookmarkString) 
	{
        ...
        $next = false;
        $folderTags = array();
		// error_log($bookmarkString);
        $lines = explode("\n", $this->sanitizeString($bookmarkString));
		
		error_log('Lines found: '.count($lines));
        foreach ($lines as $line_no => $line) 
		{
            ...
            if (preg_match('/<a/i', $line, $m2)) 
			{
				...
				// actually it is "description=" in the HTML export, not "note"
                // if (preg_match('/note="(.*?)"<\/a>/i', $line, $m5)) {
                if (preg_match('/description="(.*?)"/i', $line, $m5)) 
				{
    ...
    public static function sanitizeString($bookmarkString)
    {
        ...
        // trim comments
		/* Hack comment: This actually removed the entire bookmark string, needed to comment out this line... */
